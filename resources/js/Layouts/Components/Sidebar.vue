<template>
    <div class="hidden lg:block p-4 h-screen fixed pb-24 mt-16 overflow-hidden w-20 no-scrollbar transition-[width]
        ...
        <nav class="flex-grow overflow-auto no-scrollbar" :class="[
            ...
            <ul class="my-2 flex flex-col gap-2 items-stretch">
                <li v-for="menuItem, index in sideMenuItems" :key="index">
                    <div>
                        <div v-if="can(menuItem.permission) && menuItem.type === 'header' && !sidemenu" class="text-xs text-gray-700 uppercase font-bold mb-2 ml-4  transition-[width] overflow-hidden overflow-elipsis whitespace-nowrap">
                            {{ menuItem.label }}
                        </div>
                        <div v-if="can(menuItem.permission) && menuItem.type === 'header' && sidemenu" class="mb-2 pl-2">
                            <hr class=" border-t-2 border-gray-400 mb-4 mt-1">
                        </div>
                    </div>
    ...
    <div class="block lg:hidden p-4 fixed h-screen mt-16 overflow-hidden w-64 no-scrollbar transition-all shadow-xl
                duration-300 ease-in-out bg-[url('/images/pkt-pattern.png')] bg-contain bg-left-bottom bg-no-repeat bg-primary-surface z-50"
        :class="[
            { '-left-64': !drawer },
            { 'left-0': drawer }
