<template>
    <div class="top-0 left-0 fixed z-50 w-full">
        <div class="bg-primary-surface mx-auto pb-2">
            <div class="flex justify-between bg-gradient-to-r from-blue-600 to-teal-600 pl-2 pr-6">
                <div class="flex align-middle items-center m-2">
                    <button class="block lg:hidden mr-4 ml-3 bg-white bg-opacity-40 p-2 rounded-md shadow-md" @click="toggleDrawer" aria-label="menu-btn">
                        <BsIcon icon="bars-3" class=""></BsIcon>
                    </button>
                    <button class="hidden lg:block mr-4 ml-3 bg-white bg-opacity-40 p-2 rounded-md shadow-md" @click="toggleSideMenu" aria-label="menu-btn">
                        <BsIcon icon="bars-3" class=""></BsIcon>
                    </button>
                    <a href="/">
                        <img src="/images/logo-white.png" class="max-h-7 w-auto" name="logo" alt="app-logo"/>
                    </a>
                </div>
                <div class="flex shrink-0 align-middle items-center relative">
                    <div class="hidden md:block group relative" v-if="isEnableGlobalSearch">
                        <div class="relative flex items-center text-gray-900 bg-opacity-30 peer">
                            <BsIcon icon="magnifying-glass" class="absolute left-3 text-gray-900"></BsIcon>
                            <input 
                                type="text" 
                                placeholder="Search..." 
                                class="pl-10 pr-4 py-2 w-full rounded-md text-sm focus:outline-none focus:ring-0 bg-white bg-opacity-40 border-none placeholder-gray-800" 
                                v-model="globalSearch"
                                @keypress.enter="search"
                            >
                        </div>
                        <div class="hidden peer-focus-within:block hover:block absolute top-13 right-0 z-10 mt-4 w-64 md:w-[26rem] max-h-[24rem] overflow-y-auto origin-top-right rounded-md bg-white border border-gray-400 focus:outline-none shadow-lg" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
                            <div class="p-2 cursor-default" role="none">
                                <span class="font-bold   text-md">Search Result</span>
                                <hr class="mx-4 my-2 border-t border-gray-200">
                                <div v-for="(records, model) in globalSearchResult">
                                    <div class="sticky px-2 py-1 text-gray-700 font-semibold text-sm">{{ model }}</div>
                                    <div v-for="record in records">
                                        <a :href="record.url" class="block text-sm px-2 py-1 hover:text-primary">{{ record.value }}</a>
                                    </div>
                                </div>
                    ...
                    <div class="md:hidden" v-if="isEnableGlobalSearch">
                        <BsIconButton icon="magnifying-glass" @click="showSearch" class="text-white"></BsIconButton>
                    </div>
                    
                    <button class="group relative">
                        <el-badge :is-dot="notifications.length !== 0" :offset="[-10,10]" class="mx-2 p-2 hover:bg-white hover:bg-opacity-10 rounded-full cursor-pointer">
                            <BsIcon icon="bell" class="text-white"></BsIcon>
                        </el-badge>
                        <div class="hidden group-focus:block group-hover:block absolute top-13 mr-2 right-0 z-10 mt-4 w-64 md:w-[24rem] max-h-[24rem] overflow-y-auto origin-top-right rounded-md bg-white border border-gray-400 focus:outline-none shadow-lg" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
                            <div class="px-2 pt-2 text-start fixed bg-white rounded-t-md w-64 md:w-[23rem] max-h-[23rem] cursor-default flex justify-between items-center">
                            ...
                            <div class="py-1 mt-12" role="none">
                                <hr class="mx-4 mb-2 border-t border-gray-200">
                    ...
                    <button class="group text-start hover:bg-white hover:bg-opacity-10 focus:bg-white focus:bg-opacity-10 p-2">
                        <div class="flex flex-row items-center gap-2 ">
                            <div class="shrink-0 rounded-full p-1 bg-opacity-30 flex align-middle items-center bg-white cursor-pointer">
                                <BsProfilePicture class="w-9 h-9 rounded-full shadow-lg" :npk="user.npk"/>
                            </div>
                            <div class="w-24 truncate text-xs font-bold text-white">{{ $page.props.auth.user.name }}</div>
                            <BsIcon icon="chevron-down" class="text-white"/>
                        </div>
                        <div class="hidden group-focus:block group-hover:block absolute top-13 right-0 z-10 mt-3 w-64 origin-top-right rounded-md bg-white border border-gray-400 focus:outline-none shadow-lg" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
                            <div class="py-1" role="none">
                                <div class="p-4  flex flex-row items-start">
                                    <BsProfilePicture class="w-10 h-10 mr-2 rounded-full shadow-lg" :npk="user.npk"/>
                                    <div class="flex items-center">
                                        <div class=" flex flex-col text-gray-900">
                                            <div class="w-48 truncate text-sm font-bold">{{ $page.props.auth.user.name }}</div>
                                            <div class="w-48 truncate text-[11px]">{{ $page.props.auth.user.username }}</div>
                                            <div class="w-48 truncate text-[11px]">{{ $page.props.auth.user?.work_unit }}</div>
                                            <div class="w-48 truncate text-[11px]">{{ $page.props.auth.user?.position }}</div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <Link href="/account" class="flex flex-row px-4 py-2 text-gray-900 text-sm hover:bg-primary-surface hover:rounded-md" role="menuitem" tabindex="-1" id="menu-item-0">
                                    <BsIcon icon="user" class="mr-2"></BsIcon>
                                    Account
                                </Link>
                                <Link href="/logout" method="post" as="button" class="flex flex-row text-danger px-4 py-2 text-sm hover:bg-primary-surface hover:rounded-md w-full" role="menuitem" tabindex="-1" id="menu-item-2">
                                    <BsIcon icon="arrow-right-on-rectangle" class="mr-2"></BsIcon>
                                    Logout
                                </Link>
                            </div>
                        </div>
        ...
        <div class="h-3 bg-primary-surface relative">
            <div class="pl-0 flex-1 flex transition-[padding-left] duration-300 ease-in-out" :class="[
                { 'lg:pl-20': sidemenu },
                { 'lg:pl-64': !sidemenu }
            ]">
                <div class="px-6 right-0 w-full">
                    <div class="bg-white px-8 rounded-t-xl h-3 border border-t-gray-400 border-x-gray-400 border-b-white">
                    </div>
                </div>
            </div>
        </div>
